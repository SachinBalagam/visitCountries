      ...
      <div className="app-container">
        ...
        <ul className="countries-list-container">
          {countriesList.map(each => (
            <li key={each.id} className="each-country">
              ...
              {each.isVisited ? (
                ...
                <button
                  type="button"
                  className="button"
                  onClick={() => this.onClickChangeVisit(each.id)}
                >
                  Visit
                </button>
              )}
        ...
        <div>
          ...
          {filterCountries <= 0 ? (
            <p className="no-list-view">No Countries Visited Yet</p>
          ) : (
            <ul className="output-container">
              {filterCountries.map(each => (
                <li key={each.id}>
                  <div className="each-output-country">
                    <img
                      src={each.imageUrl}
                      alt="thumbnail"
                      className="image"
